...
def show_welcome_message():
    ...
    with st.expander("About the creator"):
        st.markdown(
            ...
            I am a Data Scientist with experience across Banking and Telecom industries.

            I built this platform to make mutual fund analysis easier to understand for everyday investors like me.

            A bit about myself: I started my investing journey back in 2018 without much guidance. Like many first time investors, I had to fumble, make mistakes, learn from them, and slowly correct my approach.
...
def home_page():
    ...
    if st.session_state.selected_funds:
        ...
        with st.spinner("eMFer is preparing your mutual fund research workspace..."):
            ...
            for idx, row in st.session_state.selected_funds_df.iterrows():
                ...
                for anomaly in nav_anomalies:
                    st.session_state.nav_anomaly_warnings.append({
                        "fund_name": row["schemeName"],
                        "date": anomaly["date"].date(),
                        "previous_nav": anomaly["previous_nav"],
                        "nav": anomaly["nav"],
                    })
        ...
        if st.session_state.nav_anomaly_warnings:
            warning_text = "\n".join(
                "- "
                f"{warning['fund_name']} around {warning['date']}: "
                f"NAV changed from {warning['previous_nav']:.2f} to {warning['nav']:.2f}."
                for warning in st.session_state.nav_anomaly_warnings
            )
            st.warning(
